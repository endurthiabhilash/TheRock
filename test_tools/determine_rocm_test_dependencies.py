...
def _load_consumer_graph(therock_dir: Path | None = None) -> dict:
    ...
    # not a subproject node. Drop it here so graph key iteration/walks see only
    # real subprojects; the CLI reads it via _load_subtree_map().
    graph.pop("subtree_map", None)
    return graph


def _load_subtree_map(therock_dir: Path | None = None) -> dict[str, list[str]]:
    """Load the subtree -> [graph keys] map from the committed consumer graph.

    Keys are external-repo source subtrees (`projects/clr`, `shared/rocroller`,
    `dnn-providers/hip-kernel-provider`); values are the one-or-more graph keys
    built from that subtree (`projects/clr` -> `hip-clr`, `ocl-clr`). Generated by
    therock_emit_consumer_graph(). Returns {} if the graph predates the map.
    """
    graph_path = _consumer_graph_path(therock_dir)
    if not graph_path.exists():
        return {}
    raw = json.loads(graph_path.read_text())
    subtree_map = raw.get("subtree_map", {})
    return {str(k): list(v) for k, v in subtree_map.items()}


def resolve_to_graph_keys(
    changed_projects: list[str], therock_dir: Path | None = None
) -> list[str]:
    """Map external-repo CI inputs to consumer-graph keys via subtree_map.

    Each incoming identifier is a source subtree path (`projects/clr`,
    `shared/rocroller`, `dnn-providers/hip-kernel-provider`). It is resolved
    through subtree_map to one-or-more graph keys, expanding fan-outs
    (`projects/clr` -> `hip-clr`, `ocl-clr`) before selection. Matching is
    case-insensitive on the subtree path.

    Inputs absent from the map fall back to `removeprefix("projects/")`, so
    callers already holding a graph key (or the legacy `projects/<name>` form for
    a name that is itself a key) still work. Unresolved names pass through so the
    downstream typo-guard in get_subprojects_to_test warns about them. The result
    is de-duplicated with order preserved.
    """
    lower_index = {
        k.lower(): v for k, v in _load_subtree_map(therock_dir).items()
    }
    resolved: list[str] = []
    for item in changed_projects:
        keys = lower_index.get(item.lower())
        if keys is not None:
            resolved.extend(keys)
        else:
            resolved.append(item.removeprefix("projects/"))
    seen: set[str] = set()
    deduped: list[str] = []
    for key in resolved:
        if key not in seen:
...
def get_subprojects_to_test(
    ...
    therock_consumer_graph.json). This function only lowercases. The CLI maps
    external-repo subtree paths (`projects/clr` -> `hip-clr`, `ocl-clr`;
    `shared/rocroller` -> `rocroller`) to graph keys via resolve_to_graph_keys()
    before calling this. The remaining hyphen/underscore skew between graph keys
    and CI test-matrix keys (`hipdnn-integration-tests` vs the graph's
    `hipdnn_integration_tests`) is a separate layer, tracked independently.
    """
...
def main():
    ...
    if changed:
        changed = resolve_to_graph_keys(changed, therock_dir)
